...
# ----------------------------------------------------------------------------
ROOTDIR = os.path.join(cfg.PATH, 'project1')
...
def read_dat(tic):
    ...
    # <COMPLETE THIS PART>
    data_file = tic + '_prc.dat'
    file_obj = open(os.path.join(DATDIR, data_file),'r')
    tic_data = file_obj.readlines()
    tic_data = [i.strip('\n') for i in tic_data]
    return tic_data
...
def line_to_dict(line):
    ...
    for column_name in COLUMNS:
        message = line[processed_length: processed_length + COLWIDTHS[column_name]]
        if column_name == 'Date':
            message = message.replace(' ', '', message.find(message.lstrip()))
        column_data[column_name] = message
        processed_length = processed_length + COLWIDTHS[column_name]
    return column_data
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    # <COMPLETE THIS PART>
    if tickers_lst == []:
        raise Exception("Error: The ticker_lst is currently empty. Please verify the file content")

    key_dic = list(tic_exchange_dic)
    for i in tickers_lst:
        if i not in key_dic:
            raise Exception("{} is not a key of tic_exchange_dic.".format(i))
...
def verify_cols(col_lst=None):
    ...
    # <COMPLETE THIS PART>
    if col_lst == []:
        raise Exception("Error: The col_lst is currently empty. Please verify the file content")

    for i in col_lst:
        if i not in COLUMNS:
            raise Exception("{} is not found in 'COLUMNS'.".format(i))
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    return_dict = {}
    for ticker_symbol in tickers_lst:
        ticker_symbol_dict = {}
        ticker_symbol_dict['exchange'] = tic_exchange_dic[ticker_symbol]

        data = []
        ticker_records = read_dat(ticker_symbol)
        for i in ticker_records:
            d = line_to_dict(i)
            data.append({key:value for key,value in d.items() if key in col_lst})

        ticker_symbol_dict['data'] = data


        return_dict[ticker_symbol] = ticker_symbol_dict
    return return_dict
